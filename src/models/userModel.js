...
const getUserById = async (id) => {
  const SQLquery = `SELECT id, name, username, email, role, created_at updated_at FROM user 
  WHERE id = ${id}`;
  const [rows] = await dbPool.query(SQLquery);
...
const createUser = async (name, username, email, role, password) => {
  const SQLquery = `INSERT INTO user (name, username, email, role, password)
  VALUES (?, ?, ?, ?, ?)`;
...
const updateUser = async (id, name, username, email, role, password) => {
  ...
  const SQLquery = `UPDATE user SET name = ?, username = ?, email = ?, role = ?, password = ? WHERE id = ${id}`;
